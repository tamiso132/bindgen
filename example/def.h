...
#define VEC_ADD(vec, add)                                                                                              \
  ...
  if((vec).len >= (vec).capacity) {                                                                                    \
    ...
    void *macro_buffer       = (void *)malloc((vec).element_len * macro_new_capacity);                                 \
...
#define VEC_REMOVE(vec, index)                                                                                         \
  {                                                                                                                    \
    ...
    u32 macro_diff      = (vec).len - 1 - index;                                                                       \
                                                                                                                       \
    memmove(&(vec).data[index], &(vec).data[macro_start_src], (vec).element_len *macro_diff);                          \
    (vec).len--;                                                                                                       \
  }
...
#define VEC_EXTEND_BY(macro_vec, macro_p_data, macro_len)                                                              \
  {                                                                                                                    \
    if((macro_vec).len + (macro_len) >= (macro_vec).capacity) {                                                        \
      ...
      void *macro_buffer       = (void *)calloc((macro_vec).element_len, macro_new_capacity);                          \
      memcpy(macro_buffer, (macro_vec).data, ((macro_vec).len) * (macro_vec).element_len);                             \
      free(macro_vec.data);                                                                                            \
      ...
      (macro_vec).capacity = macro_new_capacity;                                                                       \
                                                                                                                       ...
                                                                                                                       \
  memcpy(&VEC_GET(macro_vec, macro_vec.len), macro_p_data, macro_vec.element_len *macro_len);                          \
...
#define VEC_SET_ELEMENT(vec, add, index) memcpy(&vec.data[index], &add, vec.element_len);
